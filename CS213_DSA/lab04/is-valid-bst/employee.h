...
bool operator == (const struct employee_t &lhs, const struct employee_t &rhs) {
  ...
  if(lhs.department_name==rhs.department_name&&lhs.id_in_department==rhs.id_in_department) return true;
  return false; // dummy
}
...
std::strong_ordering operator <=> (const struct employee_t &lhs, 
  ...
  //TODO: implement <==>
  if(lhs.department_name>rhs.department_name) return std::strong_ordering::greater;
  if(lhs.department_name==rhs.department_name){
    if(lhs.id_in_department==rhs.id_in_department) return std::strong_ordering::equal;
    if(lhs.id_in_department>rhs.id_in_department) return std::strong_ordering::greater;
    if(lhs.id_in_department<rhs.id_in_department) return std::strong_ordering::less;
  } 
